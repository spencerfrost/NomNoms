...
async function migrateRecipes(users) {
  ...
  try {
    ...
    for (const filename of jsonFiles) {
      ...
      const cleanIngredients = jsonRecipe.ingredients.map(
        ({ _originalAmount, ...ingredient }) => ingredient
      );
...
async function verifyMigration() {
  ...
  try {
    ...
    if (sampleUser) {
      console.log(
        `🔍 Sample user: ${sampleUser.email} with ${sampleUser.recipes.length > 0 ? 'recipes' : 'no recipes'}`
      );
    }
    ...
    if (sampleRecipe) {
      ...
      console.log(
        `   Ingredients: ${Array.isArray(sampleRecipe.ingredients) ? 'Array (JSON)' : 'Invalid'}`
      );
