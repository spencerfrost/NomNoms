...
export default function RecipeImagePreview({ src, alt, className = '' }: RecipeImagePreviewProps) {
  ...
  if (hasError) {
    return (
      <div className={`flex flex-col items-center justify-center bg-gray-100 text-gray-500 rounded-md border border-dashed border-gray-300 ${className}`}>
        <XCircle className="h-8 w-8 mb-2" />
        <p className="text-sm text-center px-2">Failed to load image</p>
        <p className="text-xs text-center px-2 mt-1">Check if the URL is valid and publicly accessible</p>
      </div>
    )
  }
